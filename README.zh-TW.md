...
`profile.toml` 除了 `model_provider` 之外，還可以包含任意頂層 Codex 設定鍵：
...
除 `model_provider` 之外的所有鍵，都會在啟動時以 `-c KEY=VALUE` 的形式傳給 Codex。

如果 profile 目錄下存在 `models_catalog.json` 檔案，會自動加上：

```bash
-c model_catalog_json=/path/to/profiles/moonbridge/models_catalog.json
```

這對 [moon-bridge](https://github.com/ZhiYi-R/moon-bridge) 等可以產生模型目錄 JSON 的 provider 很有用。
